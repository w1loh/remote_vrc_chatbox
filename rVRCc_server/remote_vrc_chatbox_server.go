package main

import (
	"encoding/base64"
	"encoding/json"
	"fmt"
	"net/http"
	"os"
	"os/exec"
	"strings"
	"sync"
	"time"

	"github.com/atotto/clipboard"
	"github.com/getlantern/systray"
	"github.com/gorilla/websocket"
	"github.com/hypebeast/go-osc/osc"
)

type LogEvent struct {
	Type string `json:"type"`
	Addr string `json:"addr,omitempty"`
	Time string `json:"time"`
	Msg  string `json:"msg,omitempty"`
}

var (
	bcMu   sync.RWMutex
	bcSubs = map[chan LogEvent]struct{}{}
)

func bcSubscribe() chan LogEvent {
	ch := make(chan LogEvent, 64)
	bcMu.Lock()
	bcSubs[ch] = struct{}{}
	bcMu.Unlock()
	return ch
}

func bcUnsubscribe(ch chan LogEvent) {
	bcMu.Lock()
	delete(bcSubs, ch)
	bcMu.Unlock()
}

func bcPublish(ev LogEvent) {
	bcMu.RLock()
	defer bcMu.RUnlock()
	for ch := range bcSubs {
		select {
		case ch <- ev:
		default:
		}
	}
}

func logEvent(typ, addr, msg string) {
	ev := LogEvent{
		Type: typ,
		Addr: addr,
		Time: time.Now().Format("15:04:05"),
		Msg:  msg,
	}
	bcPublish(ev)
	switch typ {
	case "join":
		fmt.Printf("┃ JOIN>>  IP:%s\n", addr)
	case "quit":
		fmt.Printf("┃ QUIT>>  IP:%s\n", addr)
	case "message":
		fmt.Printf("┃ MESSAGE_%s_%s>>  %s\n", addr, ev.Time, msg)
	case "paste":
		fmt.Printf("┃ PASTE_%s_%s>>  %s\n", addr, ev.Time, msg)
	case "copy":
		fmt.Printf("┃ COPY_%s_%s>>  %s\n", addr, ev.Time, msg)
	default:
		fmt.Printf("┃ UNKNOWN_%s_%s>>  WHAT??\n", addr, ev.Time)
	}
}

var upgrader = websocket.Upgrader{
	CheckOrigin: func(r *http.Request) bool {
		return true
	},
}

func main() {
	b64icon := ""
	acquireSingleInstance()

	go showSplash()

	go func() {
		http.HandleFunc("/view", logViewHandler)
		http.HandleFunc("/logs", logStreamHandler)
		http.HandleFunc("/", newClient)
		http.ListenAndServe("0.0.0.0:41129", nil)
	}()

	displayIcon(b64icon)
}

func handleClient(conn *websocket.Conn) {
	defer conn.Close()

	for {
		_, p, err := conn.ReadMessage()
		if err != nil {
			if websocket.IsCloseError(err, websocket.CloseNormalClosure) {
				logEvent("quit", conn.RemoteAddr().String(), "")
			} else {
				fmt.Println(err)
			}
			return
		}
		var receiveMap map[string]any
		json.Unmarshal([]byte(string(p)), &receiveMap)
		mode := receiveMap["mode"]
		msg := receiveMap["textmsg"].(string)

		switch mode {
		case "nomal":
			sendOSC(msg)
			logEvent("message", conn.RemoteAddr().String(), msg)
		case "paste":
			err := clipboard.WriteAll(msg)
			if err != nil {
				fmt.Println(err)
			}
			logEvent("paste", conn.RemoteAddr().String(), msg)
		case "copy":
			s, err := clipboard.ReadAll()
			if err != nil {
				fmt.Println(err)
				s = "*error* 文字列ではないものがコピーされています"
			}
			s = strings.ReplaceAll(s, "\r\n", " ")
			s = strings.ReplaceAll(s, "\n", " ")
			if s != "" {
				payload := map[string]string{"clip": s}
				err := conn.WriteJSON(payload)
				if err != nil {
					fmt.Println(err)
				}
				logEvent("copy", conn.RemoteAddr().String(), s)
			}
		default:
			logEvent("unknown", conn.RemoteAddr().String(), "")
		}
	}
}

func newClient(w http.ResponseWriter, r *http.Request) {
	conn, err := upgrader.Upgrade(w, r, nil)
	if err != nil {
		fmt.Println(err)
		return
	}
	logEvent("join", conn.RemoteAddr().String(), "")
	go handleClient(conn)
}

func logStreamHandler(w http.ResponseWriter, r *http.Request) {
	conn, err := upgrader.Upgrade(w, r, nil)
	if err != nil {
		return
	}
	defer conn.Close()

	ch := bcSubscribe()
	defer bcUnsubscribe(ch)

	for ev := range ch {
		if err := conn.WriteJSON(ev); err != nil {
			return
		}
	}
}

func sendOSC(msg string) {
	oscClient := osc.NewClient("localhost", 9000)
	oscMsg := osc.NewMessage("/chatbox/input")
	oscMsg.Append(msg)
	oscMsg.Append(true)
	oscClient.Send(oscMsg)
}

func openGUI() {
	exec.Command("rundll32", "url.dll,FileProtocolHandler", "http://localhost:41129/view").Start()
}

func logViewHandler(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	w.Write([]byte(logViewHTML))
}

func displayIcon(b64Icon string) {
	iconBytes, err := base64.StdEncoding.DecodeString(b64Icon)
	if err != nil {
		fmt.Println(err)
		return
	}

	systray.Run(func() {
		systray.SetIcon(iconBytes)
		openBtn := systray.AddMenuItem("ログをブラウザで表示", "")
		systray.AddSeparator()
		exitBtn := systray.AddMenuItem("終了する", "")

		for {
			select {
			case <-openBtn.ClickedCh:
				openGUI()
			case <-exitBtn.ClickedCh:
				os.Exit(0)
			}
		}
	}, func() {})
}

const logViewHTML = `<!DOCTYPE html>
<html lang="ja">
<head>
<meta charset="UTF-8">
<title>rVRCc Server Log</title>
<style>
*{box-sizing:border-box;margin:0;padding:0}
body{background:#0f1117;color:#c9d1d9;font-family:'Consolas','Courier New',monospace;height:100vh;display:flex;flex-direction:column;font-size:13px}
#header{background:#161b22;padding:8px 14px;display:flex;align-items:center;gap:12px;border-bottom:1px solid #30363d;flex-shrink:0}
#title{font-weight:bold;font-size:14px;color:#e6edf3}
#status{font-size:12px;transition:color .3s}
#status.ok{color:#3fb950}
#status.ng{color:#f85149}
#log{flex:1;overflow-y:auto;padding:6px 10px}
.row{display:flex;gap:10px;padding:2px 4px;border-radius:3px;line-height:1.55}
.row:hover{background:#1c2128}
.t{color:#484f58;min-width:68px}
.k{min-width:56px;font-weight:bold}
.k.join{color:#3fb950}.k.quit{color:#d29922}.k.message{color:#58a6ff}
.k.paste{color:#bc8cff}.k.copy{color:#79c0ff}.k.unknown{color:#f85149}
.a{color:#8b949e;min-width:150px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.m{color:#e6edf3;word-break:break-all}
</style>
</head>
<body>
<div id="header">
  <span id="title">rVRCc Server Log</span>
  <span id="status" class="ng">✗ 接続中...</span>
</div>
<div id="log"></div>
<script>
const log=document.getElementById('log'),st=document.getElementById('status');
let atBottom=true;
log.addEventListener('scroll',()=>{atBottom=log.scrollHeight-log.scrollTop-log.clientHeight<40});
const labels={join:'JOIN',quit:'QUIT',message:'MSG',paste:'PASTE',copy:'COPY'};
function esc(s){return s.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}
function connect(){
  const ws=new WebSocket('ws://'+location.host+'/logs');
  ws.onopen=()=>{st.textContent='● 接続済み';st.className='ok'};
  ws.onmessage=e=>{
    const d=JSON.parse(e.data);
    const row=document.createElement('div');row.className='row';
    row.innerHTML='<span class="t">['+esc(d.time)+']</span>'
      +'<span class="k '+esc(d.type)+'">'+(labels[d.type]||d.type.toUpperCase())+'</span>'
      +'<span class="a">'+esc(d.addr||'')+'</span>'
      +(d.msg?'<span class="m">'+esc(d.msg)+'</span>':'');
    log.appendChild(row);
    if(atBottom)log.scrollTop=log.scrollHeight;
  };
  ws.onclose=()=>{st.textContent='✗ 切断 — 再接続中...';st.className='ng';setTimeout(connect,3000)};
}
connect();
</script>
</body>
</html>`
